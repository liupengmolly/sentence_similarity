...
class Configs(object):
    def __init__(self):
        ...
        # @ ----- Text Processing ----
        parser.add_argument('--word_embedding_length', type=int, default=120,
...
def linear(args,
    ...
    if input_keep_prob < 1.0 and is_train is not None:
        if tensor_util.constant_value(is_train):
...
class ModelESIM:
    ...
    def build_network(self):
        ...
        with tf.variable_scope('sent_enc',reuse=tf.AUTO_REUSE):
            ...
            bi_s1 = BiLSTM(s1_emb,self.is_train)
            bi_s2 = BiLSTM(s2_emb,self.is_train)
        ...
        with tf.variable_scope('sent_dec',reuse=tf.AUTO_REUSE):
            ctx1 = BiLSTM(inp1,self.is_train)
            ctx2 = BiLSTM(inp2,self.is_train)
