...
model_names = ['bimpm_w2v_p2v_150d_end_cv1',
               'bimpm_w2v_p2v_150d_end_cv2',
               'bimpm_w2v_p2v_150d_end_cv3',
               'bimpm_w2v_p2v_150d_end_cv4',
               'bimpm_w2v_p2v_150d_end_cv5',]
ckpts = [model_dir + name for name in model_names]
...
#     predictions =[]
#     for ckpt in ckpts:
#         prediction = bimpm_predict(sess, x_test, model, cfg, ckpt)
#         predictions.append(prediction)
#     predictions = zip(*predictions)
#     result = []
#     for i,p in enumerate(predictions):
#         num_1 = sum(p)
#         if num_1 > int(len(model_names)/2):
#             result.append(1)
#         else:
#             result.append(0)
...
with tf.Graph().as_default():
    ...
    probs = []
    for ckpt in ckpts:
        prob = bimpm_predict(sess, x_test, model, cfg, ckpt)
        probs.append(prob)
    probs = zip(*probs)
    result = []
    for i,ps in enumerate(probs):
        prob0=sum([p[0] for p in ps])
        prob1=sum([p[1] for p in ps])
        if prob0<=prob1:
            result.append(1)
        else:
            result.append(0)
