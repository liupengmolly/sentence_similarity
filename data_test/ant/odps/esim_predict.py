...
def BiLSTM(x):
    ...
    for i in range(1):
        ...
        lstm_cell = tf.contrib.rnn.DropoutWrapper(lstm_cell,output_keep_prob=cfg.dropout)
        lstm_cell_bw = tf.contrib.rnn.BasicLSTMCell(cfg.hidden_units_num)
        lstm_cell_bw = tf.contrib.rnn.DropoutWrapper(lstm_cell_bw, output_keep_prob = cfg.dropout)
...
ckpt = '/nasfile/models/user_0010214548/' + 'esim_balance_v3'

def esim_predict(sess, x_data, esim_model, cfg, checkpoint_file=None):
    if checkpoint_file:
    ...
    for db in batches:
        ...
        batch_score = sess.run(
            esim_model.predict,
            feed_dict={esim_model.sent1_token: x1_dev_b,
                       esim_model.sent2_token: x2_dev_b,
                       esim_model.is_train: False
                       })
...
with tf.Graph().as_default():
    ...
    predictions = esim_predict(sess, x_test, model, cfg, ckpt)
