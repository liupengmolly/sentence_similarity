...
from lib.model.mix.model_mix import ModelMIX
...
idf_map = dict(enumerate(json.load(open(cfg.idf_file,'r'))))

logging.basicConfig(filename="ant_mix.log" + cfg.log_name,
                    filemode="w",
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        with tf.variable_scope("ant") as scope:
            model = ModelMIX(emb.id_vector_map, scope.name)
    sess.run(tf.global_variables_initializer())

    checkpoint_dir = os.path.abspath(
        os.path.join(os.path.curdir, "ant_mix_runs", cfg.model_save_path))
    ...
    for batch in batches:
        train_loss, train_acc, train_predict,_,_ = model.train_step(sess, batch,idf_map)
