...
logging.basicConfig(filename="log_pai_disan_" + cfg.log_name,
                    filemode="w",
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        with tf.variable_scope("ant") as scope:
            model = ModelDiSAN(emb.id_vector_map, scope.name)
    sess.run(tf.global_variables_initializer())

    checkpoint_dir = os.path.abspath(
        os.path.join(os.path.curdir, "pai_disan_runs", cfg.model_save_path))
    ...
    for batch in batches:
        train_loss, train_acc, train_predict, train_logits = model.train_step(sess, batch, logging)
        train_loss_array.append(train_loss)
        train_accuracy_array.extend(train_acc)
        train_predict_array.extend(train_predict)
        train_logits_array.extend(train_logits)
        _, y = zip(*batch)
        train_y.extend(y)
        if step > 0 and step % 100 == 0:
            train_probs_array=softmax(train_logits_array)
            logging.info(
        ...
        if step > 0 and step % cfg.num_steps == 0:
            ...
            for validate_batch in validate_batches:
                ...
                logits_array.extend(logits)
            probs_array=softmax(logits_array)
