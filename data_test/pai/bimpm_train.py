...
def bilateral_match_func(in_question_repres, in_passage_repres,
    ...
    # -------------------------------------- residual network -------------------------------
    qa_aggregation_representations = tf.concat(
        [question_aware_representatins, question_lstm_representations], 2)
    pa_aggregation_representations = tf.concat(
        [passage_aware_representatins, passage_lstm_representations], 2)
...
class SentenceMatchModelGraph(object):
    ...
    def create_model_graph(self, num_classes, word_vocab=None, char_vocab=None,
        ...
        cross_entropy = -tf.reduce_sum(
            gold_matrix * tf.log(tf.clip_by_value(self.prob, 1e-10, 1.0)), axis=1)
...
with tf.Graph().as_default():
    ...
    # checkpoint_file = model_dir+'bimpm_w2v_p2v_150d_end_0.1_enhanced_avgmax'
    # saver.restore(sess, checkpoint_file)
    #    ckpt_path = model_dir+'bimpm_w2v_p2v_150d_end_4strategy_v0'
    ckpt_path = model_dir + 'bimpm_w2v_p2v_150d_end_0.1_enhanced_res'
