...
from lib.model.DIIN.tools import flatten, reconstruct, add_wd, exp_mask

import lib.model.DIIN.parameters as params
...
def double_linear_logits(args, size, bias, bias_start=0.0, scope=None, mask=None, wd=0.0,
...
def get_logits(args, size, bias, bias_start=0.0, scope=None, mask=None, wd=0.0, input_keep_prob=1.0,
    ...
    if func == 'sum':
        ...
        return double_linear_logits(args, size, bias, bias_start=bias_start, scope=scope, mask=mask,
        ...
        proj = linear([args[0]], d, False, bias_start=bias_start, scope=scope, wd=wd,
                      input_keep_prob=input_keep_prob,
                      is_train=is_train)
        ...
        return linear_logits([args[0], args[1], new_arg], bias, bias_start=bias_start, scope=scope,
                             mask=mask, wd=wd, input_keep_prob=input_keep_prob,
                             is_train=is_train)
    else:
        raise Exception()
...
def highway_layer(arg, bias, bias_start=0.0, scope=None, wd=0.0, input_keep_prob=1.0, is_train=None,
    ...
    with tf.variable_scope(scope or "highway_layer"):
        if output_size is not None:
            ...
            d = arg.get_shape()[-1]
        trans = linear([arg], d, bias, bias_start=bias_start, scope='trans', wd=wd,
                       input_keep_prob=input_keep_prob, is_train=is_train)

        trans = tf.nn.relu(trans)
        gate = linear([arg], d, bias, bias_start=bias_start, scope='gate', wd=wd,
                      input_keep_prob=input_keep_prob, is_train=is_train)
...
def highway_network(arg, num_layers, bias, bias_start=0.0, scope=None, wd=0.0, input_keep_prob=1.0,
                    is_train=None, output_size=None):
    with tf.variable_scope(scope or "highway_network"):
        prev = arg
        cur = None
        for layer_idx in range(num_layers):
            cur = highway_layer(prev, bias, bias_start=bias_start,
                                scope="layer_{}".format(layer_idx), wd=wd,
                                input_keep_prob=input_keep_prob, is_train=is_train,
...
def conv1d(in_, filter_size, height, padding, is_train=None, keep_prob=1.0, scope=None):
    with tf.variable_scope(scope or "conv1d"):
        ...
        filter_ = tf.get_variable("filter", shape=[1, height, num_channels, filter_size],
                                  dtype='float')
...
def cosine_similarity(lfs, rhs):  # [N, d]
    ...
    base = tf.sqrt(tf.reduce_sum(tf.square(lfs), axis=1)) * tf.sqrt(
        tf.reduce_sum(tf.square(rhs), axis=1))
    return dot / base
...
def dense_logits(config, args, out_size, bias, bias_start=0.0, scope=None, mask=None, wd=0.0,
    ...
    with tf.variable_scope(scope or "dense_logits"):
        ...
        if func == "tri_linear":
            ...
            out = linear(cat_args, out_size, True, bias_start=0.0, scope="dense_logit_linear",
                         squeeze=False, wd=wd, input_keep_prob=config.keep_rate, is_train=is_train)
        elif func == "mul":
            ...
            diff = args[0] - args[1]
            out = linear(diff, out_size, True, bias_start=0.0, scope="dense_logit_linear",
                         squeeze=False, wd=wd, input_keep_prob=config.keep_rate, is_train=is_train)
        ...
        if config.visualize_dense_attention_logits:
            ...
            for i in range(len(list_of_logits)):
                tf.summary.image("dense_logit_layer_{}".format(i),
                                 tf.expand_dims(list_of_logits[i], 3), max_outputs=2)
...
def fuse_gate(config, is_train, lhs, rhs, scope=None):
    with tf.variable_scope(scope or "fuse_gate"):
        ...
        if config.two_gate_fuse_gate:
            lhs_3 = linear(lhs, dim, True, bias_start=0.0, scope="lhs_3", squeeze=False,
                           wd=config.wd, input_keep_prob=config.keep_rate, is_train=is_train)
            rhs_3 = linear(rhs, dim, True, bias_start=0.0, scope="rhs_3", squeeze=False,
                           wd=config.wd, input_keep_prob=config.keep_rate, is_train=is_train)
