...
class SiameseLSTM(object):
    ...
    def BiRNN(self, x, dropout, scope, embedding_size, sequence_length,
        ...
        with tf.name_scope("fw" + scope), tf.variable_scope("fw" + scope):
            ...
            lstm_fw_cell_m = tf.nn.rnn_cell.MultiRNNCell(
                cells=stacked_rnn_fw, state_is_tuple=True
            )
        ...
        with tf.name_scope("bw" + scope), tf.variable_scope("bw" + scope):
            ...
            lstm_bw_cell_m = tf.nn.rnn_cell.MultiRNNCell(
                cells=stacked_rnn_bw, state_is_tuple=True)
    ...
    def __init__(self,
        ...
        with tf.name_scope("output"):
            self.out1 = self.BiRNN(
                self.embedded_chars1, self.dropout_keep_prob,
                "side1", embedding_size, sequence_length, hidden_units
            )
            self.out2 = self.BiRNN(
                self.embedded_chars2, self.dropout_keep_prob, "side2",
                embedding_size, sequence_length, hidden_units
            )
            self.distance = tf.sqrt(
                tf.reduce_sum(tf.square(tf.subtract(self.out1, self.out2)), 1,
                              keep_dims=True))
            self.distance = tf.div(
                self.distance,
                tf.add(
                    tf.sqrt(tf.reduce_sum(tf.square(self.out1),
                                          1,
                                          keep_dims=True)),
                    tf.sqrt(
                        tf.reduce_sum(tf.square(self.out2),
                                      1,
                                      keep_dims=True))
                )
            )
        ...
        with tf.name_scope("loss"):
            loss = self.contrastive_loss(self.input_y, self.distance,
                                         batch_size)
            ...
            for var in variables:
                ...
                tf.add_to_collection(
                    'losses', tf.multiply(tf.nn.l2_loss(var), l2_reg_lambda))
            self.loss = tf.add_n(tf.get_collection('losses'), name='reg_loss')
