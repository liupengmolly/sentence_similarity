...
class SiameseLSTMw2v(object):
    ...
    def stackedRNN(self, x, dropout, scope, embedding_size, sequence_length,
                   hidden_units):
    ...
    def __init__(self,
        ...
        with tf.name_scope("output"):
            self.out1 = self.stackedRNN(self.embedded_words1,
                                        self.dropout_keep_prob, "side1",
                                        embedding_size, sequence_length,
                                        hidden_units)
            self.out2 = self.stackedRNN(self.embedded_words2,
            ...
            self.distance = tf.sqrt(
                tf.reduce_sum(tf.square(tf.subtract(self.out1, self.out2)), 1,
                              keep_dims=True))
            self.distance = tf.div(self.distance, tf.add(
