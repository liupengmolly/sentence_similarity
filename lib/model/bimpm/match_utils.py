...
from lib.model.bimpm import layer_utils
...
def match_passage_with_question(passage_reps, question_reps, passage_mask, question_mask, passage_lengths, question_lengths,
    ...
    with tf.variable_scope(scope or "match_passage_with_question"):
        ...
        if with_maxpool_match:
            ...
            maxpooling_rep = cal_maxpooling_matching_v2(passage_reps, question_reps, maxpooling_decomp_params)
...
#     #                                                 2*options.word_embedding_length,
...
#     #                                            in_passage_repres-question_aware_representatins],2)
...
#                         reuse=False, is_training=is_training, dropout_rate=options.dropout_rate,use_cudnn=options.use_cudnn)
#             last_fw_rep = layer_utils.collect_final_step_of_lstm(fw_rep, passage_lengths - 1)
#             first_bw_rep = bw_rep[:, 0, :]
#
#             # # ---------------qa_rep max avg-----------------
...
#             # fw_rep_avg = tf.reduce_sum(fw_rep*expand_passage_mask,1)/tf.reduce_sum(expand_passage_mask,1)
#             # bw_rep_avg = tf.reduce_sum(bw_rep * expand_passage_mask,1)/tf.reduce_sum(expand_passage_mask,1)
...
#             # #------------------------------------------------
#
#             aggregation_representation.append(last_fw_rep)
#             aggregation_representation.append(first_bw_rep)
...
#                         reuse=False, is_training=is_training, dropout_rate=options.dropout_rate, use_cudnn=options.use_cudnn)
#             last_fw_rep = layer_utils.collect_final_step_of_lstm(fw_rep, question_lengths - 1)
#             first_bw_rep = bw_rep[:, 0, :]
#
#             # # ---------------qa_rep max avg-----------------
...
#             # fw_rep_avg = tf.reduce_sum(fw_rep*expand_question_mask,1)/tf.reduce_sum(expand_question_mask,1)
#             # bw_rep_avg = tf.reduce_sum(bw_rep * expand_question_mask,1)/tf.reduce_sum(expand_question_mask,1)
#             # aggregation_representation.extend([fw_rep_max,bw_rep_max,fw_rep_avg,bw_rep_avg])
#             # aggregation_dim += 2*2*options.aggregation_lstm_dim
#             # #------------------------------------------------
#
...
#
#             # ---------------qa_rep max avg-----------------
#             # fw_rep = tf.reshape(fw_rep, [-1,options.max_sentence_length,options.aggregation_lstm_dim])
#             # bw_rep = tf.reshape(bw_rep, [-1,options.max_sentence_length,options.aggregation_lstm_dim])
#             # fw_rep = tf.layers.batch_normalization(fw_rep, axis=1)
#             # bw_rep = tf.layers.batch_normalization(bw_rep, axis=1)
#
#             expand_passage_mask = tf.expand_dims(passage_mask, -1)
#             fw_rep_max = tf.reduce_max(fw_rep * expand_passage_mask, 1)
#             bw_rep_max = tf.reduce_max(bw_rep * expand_passage_mask, 1)
...
#
#             # # -----------------------------selfattention min ---------------------------------------
#             # fw_rep_attn = tf.layers.dense(fw_rep,options.aggregation_lstm_dim,activation=tf.nn.elu)
#             # bw_rep_attn = tf.layers.dense(bw_rep,options.aggregation_lstm_dim,activation=tf.nn.elu)
#             # fw_rep_attn = tf.nn.softmax(fw_rep_attn * expand_passage_mask, 1)
#             # bw_rep_attn = tf.nn.softmax(bw_rep_attn * expand_passage_mask, 1)
#             # fw_rep_attn_avg = tf.reduce_sum(fw_rep_attn * fw_rep, 1)
#             # bw_rep_attn_avg = tf.reduce_sum(bw_rep_attn * bw_rep, 1)
#             #
#             # fw_rep_min = tf.reduce_min(fw_rep * expand_passage_mask,1)
#             # bw_rep_min = tf.reduce_min(bw_rep * expand_passage_mask,1)
#             #
#             # # --------------------------------------------------------------------------------------
#
#             aggregation_representation.extend([fw_rep_max, bw_rep_max, fw_rep_avg, bw_rep_avg])
...
#             # fw_rep_attn = tf.layers.dense(fw_rep,options.aggregation_lstm_dim,activation=tf.nn.elu)
#             # bw_rep_attn = tf.layers.dense(bw_rep,options.aggregation_lstm_dim,activation=tf.nn.elu)
#             # fw_rep_attn = tf.nn.softmax(fw_rep_attn * expand_question_mask, 1)
#             # bw_rep_attn = tf.nn.softmax(bw_rep_attn * expand_question_mask, 1)
#             # fw_rep_attn_avg = tf.reduce_sum(fw_rep_attn * fw_rep, 1)
#             # bw_rep_attn_avg = tf.reduce_sum(bw_rep_attn * bw_rep, 1)
#             #
#             # fw_rep_min = tf.reduce_min(fw_rep * expand_passage_mask,1)
...
def bilateral_match_func(in_question_repres, in_passage_repres,
    ...
    with tf.variable_scope('aggregation_layer'):
        for i in xrange(options.aggregation_layer_num):  # support multiple aggregation layer
            ...
            expand_passage_mask = tf.expand_dims(passage_mask, -1)
            # # --------------------------------lstm2 res --------------------------------------------
            # agg_rep_max = tf.reduce_max(qa_aggregation_input * expand_passage_mask, 1)
            # agg_rep_avg = tf.reduce_sum(qa_aggregation_input * expand_passage_mask, 1) / (tf.reduce_sum(
            #     expand_passage_mask, 1) + 1e-10)
            # #---------------------------------------------------------------------------------------
            ...
            fw_rep_min = tf.reduce_min(fw_rep*expand_passage_mask,1)
            bw_rep_min = tf.reduce_min(bw_rep*expand_passage_mask,1)
            fw_attn = tf.layers.dense(fw_rep,options.aggregation_lstm_dim,activation=tf.nn.elu,
                                      use_bias=True)
            bw_attn = tf.layers.dense(bw_rep,options.aggregation_lstm_dim,activation=tf.nn.elu,
                                      use_bias=True)
            fw_attn = fw_attn * expand_passage_mask
            bw_attn = bw_attn * expand_passage_mask
            soft_fw_attn = tf.nn.softmax(fw_attn,1)
            soft_bw_attn = tf.nn.softmax(bw_attn,1)
            fw_attn_avg = tf.reduce_sum(soft_fw_attn*fw_rep,1)
            bw_attn_avg = tf.reduce_sum(soft_bw_attn*bw_rep,1)
            ...
            bw_rep_avg = tf.reduce_sum(bw_rep * expand_passage_mask, 1) / (tf.reduce_sum(
                expand_passage_mask, 1) + 1e-10)
            aggregation_representation.extend([fw_rep_max, bw_rep_max, fw_rep_avg, bw_rep_avg,
                                               fw_attn_avg,bw_attn_avg])
            aggregation_dim += 3 * 2 * options.aggregation_lstm_dim
            # ------------------------------------------------
            ...
            expand_question_mask = tf.expand_dims(question_mask, -1)
            # ----------------------------------attnavg min-------------------------------------------
            fw_rep_min = tf.reduce_min(fw_rep*expand_question_mask,1)
            bw_rep_min = tf.reduce_min(bw_rep*expand_question_mask,1)
            fw_attn = tf.layers.dense(fw_rep,options.aggregation_lstm_dim,activation=tf.nn.elu,
                                      use_bias=True)
            bw_attn = tf.layers.dense(bw_rep,options.aggregation_lstm_dim,activation=tf.nn.elu,
                                      use_bias=True)
            fw_attn = fw_attn * expand_question_mask
            bw_attn = bw_attn * expand_question_mask
            soft_fw_attn = tf.nn.softmax(fw_attn,1)
            soft_bw_attn = tf.nn.softmax(bw_attn,1)
            fw_attn_avg = tf.reduce_sum(soft_fw_attn*fw_rep,1)
            bw_attn_avg = tf.reduce_sum(soft_bw_attn*bw_rep,1)
            ...
            # # --------------------------------lstm2 res --------------------------------------------
            # agg_rep_max = tf.reduce_max(pa_aggregation_input * expand_question_mask, 1)
            # agg_rep_avg = tf.reduce_sum(pa_aggregation_input * expand_question_mask, 1) / (tf.reduce_sum(
            #     expand_question_mask, 1) + 1e-10)
            #---------------------------------------------------------------------------------------
            fw_rep_max = tf.reduce_max(fw_rep * expand_question_mask, 1)
            bw_rep_max = tf.reduce_max(bw_rep * expand_question_mask, 1)
            ...
            bw_rep_avg = tf.reduce_sum(bw_rep * expand_question_mask, 1) / (tf.reduce_sum(
                expand_question_mask, 1)+1e-10)
            aggregation_representation.extend([fw_rep_max, bw_rep_max,
                                               fw_rep_avg, bw_rep_avg,
                                               fw_attn_avg,bw_attn_avg])
            aggregation_dim += 3 * 2 * options.aggregation_lstm_dim
            # ------------------------------------------------
